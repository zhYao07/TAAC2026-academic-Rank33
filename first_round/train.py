...
from dataset import FeatureSchema, get_pcvr_data, NUM_TIME_BUCKETS, DENSE_STATS_FILENAME
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--amp_dtype', type=str, default='bf16',
                        ...
                        help='Mixed precision dtype on CUDA. bf16 enables '
                             'BF16 autocast without GradScaler; fp16 enables '
                             'FP16 autocast with GradScaler; fp32 disables AMP.')
    ...
    parser.add_argument('--seq_causal', action='store_true', default=False,
                        help='Whether the LongerEncoder self-attention uses a causal mask '
                             '(only effective when --seq_encoder_type=longer)')
    parser.add_argument('--longer_domains', type=str, default='',
                        help='Comma-separated seq domain names that should use LongerEncoder '
                             'regardless of --seq_encoder_type (e.g. "seq_d"). Empty (default) '
                             'means use --seq_encoder_type for every domain.')
    ...
    parser.add_argument('--use_calendar_time_features', action='store_true', default=True,
                        help='Enable sample-level and sequence-level hour/weekday/timeperiod '
                             'embeddings derived from timestamps (default on).')
    parser.add_argument('--no_calendar_time_features',
                        dest='use_calendar_time_features', action='store_false',
                        help='Disable timestamp-derived hour/weekday/timeperiod embeddings')
    parser.add_argument('--use_dense_presence_flags', action='store_true', default=True,
                        help='Append per-dimension dense value presence flags to the dense '
                             'token input (default on).')
    parser.add_argument('--no_dense_presence_flags',
                        dest='use_dense_presence_flags', action='store_false',
                        help='Disable dense value presence flags')
    parser.add_argument('--use_dense_value_norm', action='store_true', default=True,
                        help='Apply per-dimension dense value scaling using training-set stats '
                             '(default on).')
    parser.add_argument('--no_dense_value_norm',
                        dest='use_dense_value_norm', action='store_false',
                        help='Disable dense value scaling')
    parser.add_argument('--use_dense_value_log1p', action='store_true', default=True,
                        help='Apply signed log1p before dense value scaling (default on).')
    parser.add_argument('--no_dense_value_log1p',
                        dest='use_dense_value_log1p', action='store_false',
                        help='Disable signed log1p before dense value scaling')
    ...
    parser.add_argument('--use_alibi', action='store_true', default=False,
                        help='Enable ALiBi additive position bias in sequence attention')
    parser.add_argument('--alibi_bias_scale', type=float, default=1.0,
                        help='Scale factor for ALiBi distance bias (default 1.0)')
    ...
    parser.add_argument('--focal_gamma', type=float, default=2.0,
                        help='Focal Loss focusing parameter gamma '
                             '(effective only when --loss_type=focal)')
    parser.add_argument('--pair_loss_weight', type=float, default=0.05,
                        help='Weight for batch pairwise AUC loss '
                             '(0 = disabled)')
    parser.add_argument('--ema_decay', type=float, default=0.9995,
                        help='EMA decay rate for model weights '
                             '(0 = disabled, recommended 0.9995)')
    parser.add_argument('--compile', dest='compile_model', action='store_true',
                        default=False,
                        help='Wrap training forward with torch.compile '
                             '(dynamic=True). Off by default; first batch '
                             'validation stays eager unless --compile_eval is set; compile incurs startup cost.')
    parser.add_argument('--no_compile', dest='compile_model', action='store_false',
                        help='Disable training torch.compile. run.sh enables --compile by default.')
    parser.add_argument('--compile_mode', type=str, default='default',
                        choices=['default', 'reduce-overhead', 'max-autotune'],
                        help='torch.compile mode (only used when --compile is set).')
    parser.add_argument('--compile_eval', action='store_true', default=False,
                        help='Also compile validation predict(). Default is False; '
                             'validation stays eager to avoid graph-break stalls.')
    ...
    # Dormant experimental switches. They are intentionally disabled by
    # default so the current baseline path remains bitwise/logically unchanged
    # unless a future run explicitly opts in.
    parser.add_argument('--use_qgen_time_decay', action='store_true', default=False,
                        help='Optional experiment: time-decay weighted pooling '
                             'inside QueryGenerator. Off by default.')
    parser.add_argument('--qgen_time_decay_strength', type=float, default=0.0,
                        help='Decay strength for --use_qgen_time_decay. 0 keeps '
                             'the original mean-pooling behavior.')
    parser.add_argument('--use_target_aware_ns_gate', action='store_true', default=False,
                        help='Optional experiment: target-aware gating on NS tokens. '
                             'Off by default.')
...
def main() -> None:
    ...
    dense_stats_path = os.path.join(args.ckpt_dir, DENSE_STATS_FILENAME)
    train_loader, valid_loader, pcvr_dataset = get_pcvr_data(
        ...
        use_dense_value_norm=args.use_dense_value_norm,
        use_dense_value_log1p=args.use_dense_value_log1p,
        dense_stats_path=dense_stats_path if args.use_dense_value_norm else None,
    )
    ...
    model_args = {
        ...
        "use_dense_presence_flags": args.use_dense_presence_flags,
        "rank_mixer_mode": args.rank_mixer_mode,
        "use_rope": args.use_rope,
        "rope_base": args.rope_base,
        "use_alibi": args.use_alibi,
        "alibi_bias_scale": args.alibi_bias_scale,
        ...
        "longer_domains": [
            d.strip() for d in args.longer_domains.split(',') if d.strip()
        ],
        "use_qgen_time_decay": args.use_qgen_time_decay,
        "qgen_time_decay_strength": args.qgen_time_decay_strength,
        "use_target_aware_ns_gate": args.use_target_aware_ns_gate,
    }
