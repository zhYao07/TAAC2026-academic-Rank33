"""PCVRHyFormer v14 inference script (uploaded by the contestant into the
...
# ckpt directory.
#
# These usually match the argparse defaults in ``train.py``. Optional features
# that were added after older checkpoints existed may deliberately fall back to
# the old disabled shape when ``train_config.json`` lacks the key.
#
...
_FALLBACK_MODEL_CFG = {
    ...
    # Optional feature added after older checkpoints existed; fallback False
    # preserves compatibility when train_config.json lacks this key.
    'use_calendar_time_features': False,
    'use_dense_presence_flags': False,
    ...
    'user_ns_tokens': 0,
    'item_ns_tokens': 0,
    # Empty list means "use --seq_encoder_type for every domain". When the
    # training run actually used --longer_domains, train_config.json will
    # carry the raw CLI string (e.g. "seq_d"), which ``resolve_model_cfg``
    # parses into the list form expected by ``PCVRHyFormer``.
    'longer_domains': [],
    # Dormant optional experiments; default False keeps older checkpoints
    # compatible and preserves baseline inference when train_config lacks them.
    'use_qgen_time_decay': False,
    'qgen_time_decay_strength': 0.0,
    'use_target_aware_ns_gate': False,
}

_FALLBACK_SEQ_MAX_LENS = 'seq_a:256,seq_b:256,seq_c:512,seq_d:512'
_FALLBACK_BATCH_SIZE = 256
_FALLBACK_NUM_WORKERS = 16
_FALLBACK_TIME_TZ_OFFSET_HOURS = 8.0
_FALLBACK_USE_DENSE_VALUE_NORM = False
_FALLBACK_USE_DENSE_VALUE_LOG1P = True
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        ...
        if key == 'longer_domains':
            # ``train.py`` stores the raw CLI string (e.g. "seq_d" or
            # "seq_c,seq_d"), but ``PCVRHyFormer`` expects ``List[str]``.
            # Older ckpts that pre-date this flag simply lack the key and
            # fall back to the empty list (= every domain uses the global
            # ``seq_encoder_type``).
            raw = train_config.get(key, _FALLBACK_MODEL_CFG[key])
            if isinstance(raw, str):
                cfg[key] = [d.strip() for d in raw.split(',') if d.strip()]
...
def _batch_to_model_input(
    ...
    for domain in seq_domains:
        ...
        seq_day_type_feats[domain] = device_batch.get(
            ...
            torch.zeros(B, L, 2, dtype=torch.long, device=device))
...
def main() -> None:
    ...
    time_tz_offset_hours = float(
        train_config.get('time_tz_offset_hours', _FALLBACK_TIME_TZ_OFFSET_HOURS))
    use_dense_value_norm = bool(
        train_config.get('use_dense_value_norm', _FALLBACK_USE_DENSE_VALUE_NORM))
    use_dense_value_log1p = bool(
        train_config.get('use_dense_value_log1p', _FALLBACK_USE_DENSE_VALUE_LOG1P))
    dense_stats_path = (
        os.path.join(model_dir, DENSE_STATS_FILENAME)
        if use_dense_value_norm else None
    )
    ...
    # Compile predict() when training used torch.compile, so inference
    # follows the same fused-kernel / numerical path as validation.
    _compile_model = bool(train_config.get('compile_model', False))
    _compile_mode = str(train_config.get('compile_mode', 'default'))
    predict_fn = model.predict
    if _compile_model and hasattr(torch, 'compile'):
        logging.info(f"Compiling model.predict with torch.compile(mode={_compile_mode}, dynamic=True)")
        predict_fn = torch.compile(model.predict, mode=_compile_mode, dynamic=True)
    else:
        logging.info("Inference uses eager mode (compile_model not set or unavailable).")
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            logits, _ = predict_fn(model_input)
