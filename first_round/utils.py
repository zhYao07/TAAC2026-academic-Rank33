...
class ModelEMA:
    ...
    Maintains shadow copies of all parameters, updated after each training
    step as: ``shadow = decay * shadow + (1 - decay) * param``.

    Args:
        model: Model whose parameters are tracked.
        decay: EMA decay rate.
        exclude_names: Optional set of parameter names to exclude from EMA
            tracking. Excluded params are left untouched by ``update``,
            ``apply_shadow``, and ``restore``. Typical use: pass sparse
            (embedding) parameter names so that EMA only smooths dense
            (MLP / attention) weights.

    Usage in the trainer:
    - After each optimizer step: ``ema.update(model)``
    - Before evaluation: ``ema.apply_shadow(model)`` (swap in EMA weights)
    - After evaluation + checkpoint: ``ema.restore(model)`` (swap back)
    """
    ...
    def __init__(
        ...
        for name, param in model.named_parameters():
            if param.requires_grad and name not in _exclude:
    ...
    def apply_shadow(self, model: nn.Module) -> None:
        """Swap model params with EMA shadow (save originals in backup)."""
        self.backup = {}
        for name, param in model.named_parameters():
            if name in self.shadow:
...
def set_seed(seed: int) -> None:
    ...
    # Speed-oriented defaults for training; deterministic mode can be much slower.
