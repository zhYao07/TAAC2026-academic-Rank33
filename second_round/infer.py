...
_FALLBACK_MODEL_CFG = {
    ...
    'seq_hash_bucket_size': 0,
    'seq_hash_gate_init': -0.75,
    'ns_hash_bucket_size': 0,
    'ns_hash_gate_init': -0.75,
    'ns_tokenizer_type': 'rankmixer',
    'user_ns_tokens': 0,
    'item_ns_tokens': 0,
    # v22 SupCon head. Must match train.py so a checkpoint trained with SupCon
    # rebuilds with the projection head present (it is unused at inference).
    'use_supcon': False,
    'supcon_proj_dim': 0,
    # v25 query pooling. Must match train.py so the checkpoint (which has the
    # DIN-MLP poolers + item_target_proj) rebuilds with identical shapes.
...
def _batch_to_model_input(
    ...
    time_feats = device_batch.get(
        'time_feats',
        torch.zeros(B0, 3, dtype=torch.long, device=device))
...
def main() -> None:
    ...
    # word2vec compressed-vocab slots (v28): read sizes from train_config so the
    # W2VSeqEmbedding modules are built with matching K, then strict-loaded.
    seq_w2v_slots = train_config.get('seq_w2v_slots', None)
    ns_w2v_slots = train_config.get('ns_w2v_slots', None)
