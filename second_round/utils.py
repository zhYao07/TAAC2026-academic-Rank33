...
def supcon_loss(
    ...
    pos_anchor_only: bool = False,
    pos_label: int = 1,
) -> torch.Tensor:
    """Supervised Contrastive loss (Khosla et al. 2020) over a batch.

    For each anchor ``i``, the positives are the *other* samples sharing its
    label; the negatives are all samples with a different label. Pulls
    same-outcome representations together and pushes different-outcome ones
    apart. Operates on whatever representation is passed in (here: the final
    pre-classifier output, which already fuses user / item / sequence).

    ``pos_anchor_only`` (v23): only the positive class (``pos_label``, the rare
    conversions) serve as anchors. The denominator (the contrastive
    normalization) still runs over *all* other samples, so the abundant
    negatives still provide the "push away" signal — only the averaged loss is
    restricted to converted anchors. This avoids the ill-posed "pull ~1000
    diverse non-converted samples together" objective that otherwise pins the
    loss near ``ln(B)`` under heavy class imbalance.

    Rare-positive handling (important for CVR, where label==1 is sparse):
    anchors with no same-label partner are excluded; a batch with no usable
    anchor returns a graph-connected zero (keeps DDP / grad flow happy).

    Args:
        z: (B, d) representations (will be L2-normalized here).
        labels: (B,) integer/binary class labels.
        temperature: softmax temperature (typical 0.05-0.2).
        pos_anchor_only: restrict anchors to the positive class.
        pos_label: which label is the positive class (default 1 = converted).

    Returns:
        Scalar SupCon loss (graph-connected 0 when no usable anchor).
    """
    ...
    # Denominator runs over all a != i (exclude self).
    exp_sim = torch.exp(sim).masked_fill(self_mask, 0.0)
    log_prob = sim - torch.log(exp_sim.sum(dim=1, keepdim=True) + 1e-12)

    labels_col = labels.contiguous().view(-1, 1)
    pos_mask = (labels_col == labels_col.t()) & (~self_mask)  # same label, not self
    pos_count = pos_mask.sum(dim=1)                            # (B,)

    # Mean log-prob over the positives of each anchor.
    pos_log_prob = (log_prob * pos_mask.float()).sum(dim=1) / pos_count.clamp(min=1)

    # Which rows count as anchors.
    anchor_mask = pos_count > 0
    if pos_anchor_only:
        anchor_mask = anchor_mask & (labels.view(-1) == pos_label)
    anchor_mask = anchor_mask.float()
    denom = anchor_mask.sum()
    if denom.item() == 0:
        # No usable anchor in this batch (e.g. zero/one converted sample).
        # Return a graph-connected zero so the projection head still receives a
        # (zero) gradient — otherwise DDP raises "parameters not used in loss".
        return z.sum() * 0.0
    return -(pos_log_prob * anchor_mask).sum() / denom


class DenseEMA:
    """Exponential moving average over a fixed set of DENSE parameters only.

    Sparse embedding tables are deliberately excluded: they are updated
    sparsely (only ids seen in a batch get gradients) and are periodically
    cold-restarted by the reinit mechanism, so a full-table EMA would average
    in stale/untrained rows and break on reinit (tensor identity changes).
    EMA is therefore applied solely to the densely-updated parameters returned
    by ``model.get_dense_params()``.

    Usage:
        ema = DenseEMA(dense_params, decay=0.9995)
        ...
        opt.step(); ema.update()          # after each optimizer step
        ...
        ema.store(); ema.copy_to()         # before eval / save
        auc = evaluate(...)
        ema.restore()                      # after eval / save (back to raw)
    """

    def __init__(self, params, decay: float = 0.9995) -> None:
        self.decay = decay
        # keep references to the live parameters (so copy_to/restore can write
        # back in place) and a detached fp32 shadow for each.
        self._params = [p for p in params if p.requires_grad]
        self.shadow = [p.detach().clone().float() for p in self._params]
        self._backup = None
        self.num_updates = 0
        logging.info(
            f"DenseEMA: tracking {len(self._params)} dense tensors "
            f"({sum(p.numel() for p in self._params):,} params), decay={decay}")

    @torch.no_grad()
    def update(self) -> None:
        """One EMA step: shadow = decay*shadow + (1-decay)*param."""
        self.num_updates += 1
        d = self.decay
    ...
    @torch.no_grad()
    def store(self) -> None:
        """Snapshot current raw params so they can be restored after eval."""
        self._backup = [p.detach().clone() for p in self._params]

    @torch.no_grad()
    def copy_to(self) -> None:
        """Load EMA (shadow) weights into the live params (for eval/save)."""
        for s, p in zip(self.shadow, self._params):
            p.data.copy_(s.to(dtype=p.dtype))

    @torch.no_grad()
    def restore(self) -> None:
        """Restore the raw params snapshotted by ``store``."""
    ...
    def load_state_dict(self, sd: Dict[str, Any]) -> None:
        ...
        for s, saved in zip(self.shadow, sd['shadow']):
            s.copy_(saved.to(s.device))
